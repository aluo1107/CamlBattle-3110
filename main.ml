...
let main () =
  ...
  else print_string ("Welcome " ^ name ^ " to CamlBattles!");
  print_string " Please select from fire, water, air, or earth ";
  let input_player_el = read_line () in
  let player_caml = caml_init (welcome_game input_player_el) in
  let enemy_caml = ai_caml player_caml in
  let player_stage =
    let () =
      print_string
        " Please choose from volcano, ocean, jungle, cloud kingdom. \
         Note: your biome can affect your attack power "
    in
    let input_biome = read_line () in
    pick_biome input_biome
  in
  let game_state =
    {
      player = player_caml;
