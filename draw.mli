...
(*[clear_window color] takes in an rgb color and colors the background
  based on that color*)
...
(*[caml_type caml] takes in the caml's element and returns the color it
  should be*)
val camel_type : string -> Graphics.color

(*[draw_user_caml color] draws the user caml based on the element chosen*)
val draw_user_caml : Graphics.color -> unit

(*[draw_enemy color] draws the enemy of the user caml on the screen. It
  draws the enemy based on the opposite element of the user's chosen
  element*)
val draw_enemy : Graphics.color -> unit

(*[user_board color] draws the player board that will keep track of the
  player stats including the hp, exp, and level*)
val user_board : Graphics.color -> State.t -> unit

(*[enemy_board color] draws the enemy board that will keep track of the
  enemy stats including the enemy's hp, exp, and level*)
